...
def ttest_pvalue(df, label_column):
    label_0_data = df.loc[df[label_column] == 0, :].drop(label_column, axis=1)
    label_1_data = df.loc[df[label_column] == 1, :].drop(label_column, axis=1)
    
    # t-test
    p_values = []
    for col in df.drop(label_column, axis=1):
        statistic, p_value = stats.ttest_ind(label_0_data.loc[:, col].dropna(), label_1_data.loc[:, col].dropna(), equal_var=True)
